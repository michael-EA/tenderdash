...
func TestReactorWithEvidence(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		//ev, _ := types.NewMockDuplicateVoteEvidenceWithValidator(1, defaultTestTime, privVals[vIdx], cfg.ChainID(), btcjson.LLMQType_5_60, state.Validators.QuorumHash)
...
func TestReactorValidatorSetChanges(t *testing.T) {
	...
	///
	fmt.Printf("All peers info:\n")
	for i, state := range states {
		proTxHash, _ := state.privValidator.GetProTxHash(context.Background())
		fmt.Printf("validator info: index=%d proTxHash=%s quorumHash=%s\n", i, proTxHash.ShortString(), state.Validators.QuorumHash.String())
	}
	///
	...
	for i := 4; i < len(states); i++ {
		state := states[i]
		proTxHash, _ := state.privValidator.GetProTxHash(context.Background())
		pv := state.privValidator.(*privval.FilePV)
		var qhs []string
		for k := range pv.Key.PrivateKeys {
			qhs = append(qhs, k[:6])
		}
		var hs []string
		for h := range pv.Key.UpdateHeights {
			hs = append(hs, h)
		}
		fmt.Printf("[DEBUG] proTxHash=%s qhs=%v hs=%v \n", proTxHash.ShortString(), qhs, hs)
	}
	...
	fmt.Printf("[DEBUG] checkpoint 02 \n")
	valsUpdater.info()
	...
	fmt.Printf("[DEBUG] checkpoint 03 \n")
}
...
func (u *validatorUpdater) info() {
	proTxHashes := u.states[0].Validators.GetProTxHashes()
	for _, proTxHash := range proTxHashes {
		j := u.stateMap[proTxHash.String()]
		fmt.Printf("[DEBUG] index=%d proTxHash=%s\n", j, proTxHash.String())
	}
}
...
func (u *validatorUpdater) updateStatePrivVals(privValUpdate *privValUpdate, height int64) {
	fmt.Printf("update state at %d quorumHash=%s newProTxHashes=%v\n", height, privValUpdate.quorumHash.String(), privValUpdate.newProTxHashes)
...
func waitFor(states []*State, proTxHashes []crypto.ProTxHash, quorumHash crypto.QuorumHash) {

	fmt.Printf("staring waitFor operation\n")
	proTxHashMap := make(map[string]struct{})
	for _, proTxHash := range proTxHashes {
		proTxHashMap[proTxHash.String()] = struct{}{}
	}
	...
	for _, state := range states {
		...
		if ok {
			stateCheck = append(stateCheck, state)
		}
	}
	...
	go func(states []*State) {
		for len(states) > 0 {
			var tmp []*State
			for _, state := range states {
				_, err := state.privValidator.GetPrivateKey(context.Background(), quorumHash)
				if err != nil {
					proTxHash, _ := state.privValidator.GetProTxHash(context.Background())
					fmt.Printf("validator %s push back\n", proTxHash.ShortString())
	...
	case <-time.After(time.Second * 5):
		fmt.Printf("waitFor timeout\n")
	case <-doneCh:
		fmt.Printf("waitFor done successful\n")
		return
	}
}
...
func validate(states []*State) {
	...
	for _, state := range states {
		...
		//if len(state.Validators.Validators) != currValidatorCount {
		//	panic("they should all have the same initial validator count")
		//}
		//if !currValidators.Equals(state.Validators) {
		//	panic("all validators should be the same")
		//}
